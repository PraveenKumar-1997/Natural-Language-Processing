...
class RnnDecoder(nn.Module):
    def __init__(self, trg_vocab, embedding_dim, hidden_units):
        ...
        self.layer1 = nn.Linear(hidden_units, hidden_units)
        self.layer2 = nn.Linear(hidden_units, hidden_units)
        self.final = nn.Linear(hidden_units, 1)
    ...
    def compute_attention(self, dec_hs, enc_output):
        ...
        ### TODO ###
        step_1 = dec_hs.permute(1,0,2)
        step_2 = enc_output.permute(1,0,2)
        out_layer_1 = self.layer1(step_1)
        out_layer_2 = self.layer2(step_2)
        final_out = F.tanh(out_layer_1 + out_layer_2)
        attention_weights = F.softmax(self.final(final_out), dim=1)
        context_vector = torch.sum(attention_weights * step_2, dim=1)
    ...
    def forward(self, x, dec_hs, enc_output):
        ...
        concatenate = torch.cat((embedding,context_vector), dim = 2)
        output, dec_hs = self.GRU(concatenate)
        out = self.out(output)
        fc_out = out.squeeze(1)
...
def decode_rnn_model(encoder, decoder, src, max_decode_len, device):
    ...
    encoder_out, encoder_hidden_ = encoder(src)
    decoder_hidden_ = encoder_hidden_

    for idx in range(1, max_decode_len):
        preds, decoder_hidden_, _ = decoder(dec_input.to(device), decoder_hidden_.to(device), encoder_out.to(device))
        curr_predictions[:,idx,:] = preds
        dec_input = torch.argmax(preds, dim=1).unsqueeze(1)
        curr_output[:, idx] = dec_input.squeeze(1)
...
def create_positional_embedding(max_len, embed_dim):
    ...
    for pos in range(shape[0]):
      for i2 in range(shape[1]):
        if i2 % 2 == 0:  # If i2 is even
          pe[pos][i2] = math.sin(pos/(10000**(i2/embed_dim)))
        else:
          pe[pos][i2] = math.cos(pos/(10000**((i2-1)/embed_dim)))
...
class TransformerEncoder(nn.Module):
    def __init__(self, src_vocab, embedding_dim, num_heads,
        ...
        # Dropout layer
        self.dropout_layer = nn.Dropout().to(device)
        # Initialize a nn.TransformerEncoder model (you'll need to use embedding_dim,
        # num_layers, num_heads, & dim_feedforward here)
        tranformer_encoder_layer = nn.TransformerEncoderLayer(nhead=num_heads, dim_feedforward=dim_feedforward, d_model=embedding_dim).to(device)
        self.Transformer = nn.TransformerEncoder(tranformer_encoder_layer, num_layers = num_layers).to(device)
    ...
    def forward(self, x):
        ...
        ### TODO ###
        x = x.to(self.device)
        src_key_padding_mask = self.make_src_mask(x)
        #print(src_key_padding_mask)
        first_part = self.embedding(x)
        second_part = self.position_embedding[:x.size(0)]
        t = first_part + second_part
        temp = self.dropout_layer(t)
        output = self.Transformer(temp, mask=None, src_key_padding_mask = src_key_padding_mask)
        return output
...
class TransformerDecoder(nn.Module):
    def __init__(self, trg_vocab, embedding_dim, num_heads,
        ...
        # Dropout layer
        self.dropout_layer = nn.Dropout().to(self.device)
        # Initialize a nn.TransformerEncoder model (you'll need to use embedding_dim,
        # num_layers, num_heads, & dim_feedforward here)
        tranformer_decoder_layer = nn.TransformerDecoderLayer(nhead=num_heads, dim_feedforward=dim_feedforward, d_model=embedding_dim).to(self.device)
        self.Transformer = nn.TransformerDecoder(tranformer_decoder_layer, num_layers = num_layers).to(self.device)
    ...
    def forward(self, dec_in, enc_out):
        ...
        ### TODO ###
        first_part = self.embedding(dec_in).to(self.device)   
        second_part = self.position_embedding[:dec_in.shape[0],:].to(self.device)   
        t = self.dropout_layer(first_part + second_part).to(self.device)   
        trg_mask = self.generate_square_subsequent_mask(dec_in.shape[0]).to(self.device)   
        final = self.Transformer(t, memory = enc_out, tgt_mask = trg_mask).to(self.device)   
        output = self.out(final).to(self.device)   
        return output
...
def decode_transformer_model(encoder, decoder, src, max_decode_len, device):
    ...
    for idx in range(0, max_decode_len-1):
        dec_input = curr_output[:,:idx+1].transpose(0,1)
        out = decoder(dec_input.int().to(device), enc_output)[-1]
        curr_predictions[:,idx+1,:] = out
        preds = torch.argmax(out,dim=1)
        curr_output[:, idx+1] = preds
